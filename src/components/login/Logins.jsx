...
const Logins = () => {
  ...
  return (
    <section className="fixed h-full w-full flex">
<div className={`${styles.container} logins w-[23rem]  md:w-[35rem] m-auto bg-transparent py-8 px-4 md:px-6 rounded-md`}>
        ...
        <form onSubmit={handleSumbit} className='text-center'>
            <div className=' items-center flex bg-white px-2 rounded-md'>
                <FaUserAlt size={20} />
                <input className='flex-1 text-lg py-2 px-4 outline-none' name='email'  type="email" placeholder='Email' onChange={handleChange} required/>
            </div>
            <div className=' items-center flex bg-white px-2 mt-6 rounded-md'>
                <RiLockPasswordFill size={20}/>
                <input className='flex-1 text-lg py-2 px-4 outline-none' name='password'  type="password" placeholder='Password' onChange={handleChange} required/>
            </div>
